...
> **当前状态：代码基线仍是 WWIIHexV0 二战 hex 战棋工程，已沉淀 Hex / Region / Theater / FrontLine / WarDeployment / Command / AI Agent 管线。项目路线按 v5.0-v5.9 维多利亚时代历史策略迁移规划推进，目标产品暂定为 `蒸汽帝国 Agent` / `Steam & Empire Agent Strategy`。v5.1 多国家 / 通用回合 / 外交敌我判断基础切片已推送并通过 GitHub Actions 结果包验收；v5.2 已接入默认 `black_sea_crisis_1853` / `黑海危机 1853` 数据入口、维多利亚势力/模板/人物/地形 JSON、MapEditor 默认资源桥和 scenario 数据驱动胜利条件；v5.3 已开始接入 typed logistics tags、铁路移动、港口补给锚点、远征 depot 补给锚点、炮兵攻城修正、维多利亚组件 schema 和单位/生产玩家可见显示适配；v5.4 已接入动员/预算动作、铁路工程、野战工事、港口工程、远征 depot 工程和围城 depot 工程起步切片；v5.5-v5.6 已接入最小 `DiplomaticPlay` 创建、支持/反对列表加入、AI 支持/反对/中立回应解释记录、整轮推进、deadline 宣战尝试、`Offer concession` 让步收束和结算记录、已升级战争的最小 `Negotiate truce` 停战结算与 2 回合停战冷却、冷却期第三方外交危机支持站队约束、最小 `Impose blockade` 封锁命令、objective-backed 动态外交战争目标胜利桥、`weakenPrestige` 战争支持阈值胜利桥，以及战争支持影响 Cabinet 姿态/外交风险阈值的最小反馈。源码命名、测试夹具和部分 legacy 数据仍保留阿登、Germany / Allies、Division、Panzer 等二战语义。当前工作流默认不跑本机 Xcode / XCTest / 模拟器测试，只按 `md/test/test.md` 做轻量检查，重验证交给 GitHub Actions。**
...
- 经济工业与科技变迁是后续重点：国库、工业产能、铁路运输力、船运量、建设能力、科技/改革修正都应进入可玩的预算和规则取舍，避免只停留在背景叙事。
- 设计可参考《维多利亚 3》的工业化、科技扩散、外交博弈、财政压力和社会反馈，但本项目面向手机端，首版必须控制操作密度、回合结算成本、界面信息量和 AI 计算复杂度，不照搬 PC 级完整全球市场、人口模拟或多层商品链。
...
├── Data/          — 场景数据（DataLoader、ScenarioDefinition JSON、black_sea_crisis_1853_*、victorian_*、legacy ardennes_*）
...
| 维多利亚围城修正 | 🚧 炮兵攻击城市/要塞有额外攻城加成，完整围城状态机未做 |
| 侧翼/背后加成 | ✅ |
| 占领系统（城市控制权变更） | ✅ |
| 补给系统（supplied / lowSupply / encircled） | ✅ |
| 维多利亚物流标签 | 🚧 黑海危机 key hex 可加载 port / rail / telegraph / expeditionaryDepot / siegeDepot 等标签；港口和远征 depot 可作为己方或 allied / coBelligerent 补给与安全撤退锚点，`.blockaded` 关系会禁用目标方港口/远征 depot 锚点 |
| 包围判定与惩罚 | ✅ |
