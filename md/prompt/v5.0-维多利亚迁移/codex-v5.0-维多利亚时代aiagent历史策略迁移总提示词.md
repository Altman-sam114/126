...
- v5.1 切片已把 `Faction` 扩展为 legacy Germany / Allies 加 Britain、France、Russia、Ottoman、Austria、Sardinia、Neutral，并新增通用 `humanAction` / `aiAction` / `diplomacyResolution`、`turnOrder`、`humanControlledFactions` 和 `DiplomacyState.canAttack` / `canEnterTerritory` 入口；v5.2 已把默认数据入口切到黑海危机；v5.3 已开始接入 typed `logisticsTags`、铁路移动、同盟港口补给锚点、炮兵攻城修正、维多利亚 `ComponentType` case、黑海单位模板 schema 和第一层单位/生产显示 adapter；v5.4 已开始把默认生产 taxonomy 切到维多利亚 `ProductionKind` case，并新增 `EconomyCommand` 动员/预算动作、铁路工程、野战工事和港口工程建设入口；v5.5-v5.6 已开始接入最小 `DiplomaticPlay` 创建、支持/反对列表加入、AI 支持/反对/中立解释记录、整轮升级、deadline 宣战尝试、`Offer concession` 让步收束、已升级战争的 `Negotiate truce` 停战结算与 2 回合停战冷却、冷却期第三方支持站队约束、objective-backed dynamic warGoal 胜利桥和 `weakenPrestige` warSupport 阈值胜利桥。默认数据和部分 UI/测试仍保留 legacy Germany / Allies。
...
- 当前经济资源内部字段仍是 `manpower/industry/supplies` 兼容名；默认生产项已暴露 `lineInfantryCorps`、`guardBrigade`、`cavalryBrigade`、`siegeArtilleryBattery`、`supplyConvoy`，legacy `panzerDivision` 等 case 仍保留作旧存档和测试兼容；预算动作已有 `mobilizeReserves`、`raiseWarLoan`、`buySupplies`，建设动作已有 `railway`、`fieldWorks` 与 `portWorks`，但还不是完整财政/建设系统。
