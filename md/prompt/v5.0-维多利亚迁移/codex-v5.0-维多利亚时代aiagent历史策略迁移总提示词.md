...
- v5.1 切片已把 `Faction` 扩展为 legacy Germany / Allies 加 Britain、France、Russia、Ottoman、Austria、Sardinia、Neutral，并新增通用 `humanAction` / `aiAction` / `diplomacyResolution`、`turnOrder`、`humanControlledFactions` 和 `DiplomacyState.canAttack` / `canEnterTerritory` 入口；v5.2 已把默认数据入口切到黑海危机；v5.3 已开始接入 typed `logisticsTags`、铁路移动、同盟港口与远征 depot 补给锚点、炮兵攻城修正、维多利亚 `ComponentType` case、黑海单位模板 schema 和第一层单位/生产显示 adapter；v5.4 已开始把默认生产 taxonomy 切到维多利亚 `ProductionKind` case，并新增 `EconomyCommand` 动员/预算动作、铁路工程、野战工事、港口工程、远征 depot 工程和围城 depot 工程建设入口；v5.5-v5.6 已开始接入最小 `DiplomaticPlay` 创建、支持/反对列表加入、AI 支持/反对/中立解释记录、整轮升级、deadline 宣战尝试、`Offer concession` 让步收束、已升级战争的 `Negotiate truce` 停战结算与 2 回合停战冷却、冷却期第三方支持站队约束、最小 `imposeBlockade` 封锁命令、objective-backed dynamic warGoal 胜利桥和 `weakenPrestige` warSupport 阈值胜利桥。默认数据和部分 UI/测试仍保留 legacy Germany / Allies。
...
- 当前经济资源内部字段仍是 `manpower/industry/supplies` 兼容名；默认生产项已暴露 `lineInfantryCorps`、`guardBrigade`、`cavalryBrigade`、`siegeArtilleryBattery`、`supplyConvoy`，legacy `panzerDivision` 等 case 仍保留作旧存档和测试兼容；预算动作已有 `mobilizeReserves`、`raiseWarLoan`、`buySupplies`，建设动作已有 `railway`、`fieldWorks`、`portWorks`、`expeditionaryDepotWorks` 与 `siegeDepotWorks`，但还不是完整财政/建设系统。
...
当前源码已先落地最小 `supportDiplomaticPlay` 和 `respondToDiplomaticPlay`：未加入且非主当事方的 faction 可选择支持 issuer 或 target，只写入 `backers` / `opposingBackers`，不改变外交关系、不自动参战、不刷新前线/部署；AI 可经 `RulerAgent -> TurnManager -> Command.diplomacy(.respondToDiplomaticPlay) -> RuleEngine` 留下支持、反对或中立解释记录。`RulerAgent` 已开始读取 warSupport 来影响 Cabinet posture、attackThresholdAdjustment、reserveBias 和外交危机回应倾向；低 warSupport 可让 AI 中立并在 rationale 中说明避免升级。`offerConcession` 已开始写入 `settlementRecord`，对让步方/受益方做最小 warSupport 结算，记录值按 0-100 钳制后的实际变化，并阻止同回合重开同一危机。`negotiateTruce` 已开始支持已升级战争的 play 由 issuer 或 target 发起最小停战，把 cross-side `atWar` 关系写为 `truce`、刷新前线/部署，并在默认 2 回合冷却期内阻止停战双方直接宣战、重开危机或借第三方支持站队绕过停战；冷却期后可重新进入外交/战争流程。`imposeBlockade` 已开始支持 action phase 中对 hostile / neutral 或已过停战冷却的合法目标写入 `.blockaded` 关系、外交日志并刷新派生层；当前它不是完整海军封锁、补给惩罚、港口吞吐或 off-map sea lane 系统。已升级为战争的 objective-backed play warGoal 会进入 `VictoryRules`；`weakenPrestige` 可在目标方主国家 warSupport 跌到 35 或以下时触发最小外交战争目标胜利。但完整条件交换、多方和平谈判、赔款/割地、完整威望/恶名系统和完整动态战争目标仍是后续任务。
...
- 封锁命令当前只改变外交关系状态；开战、移动、占领、补给惩罚、港口吞吐和海上通道效果必须由后续规则切片单独接入。
...
- 至少一个 AI 国家能支持、反对或保持中立，并留下解释记录；当前最小手动/UI 支持命令不能替代该 AI 解释验收。
