...
// v0 TurnManager: orchestrates the legacy AI turn. Does not implement rules.
...
struct TurnManager {
    ...
    static func contextSummary(_ context: AgentContext) -> String {
        "\(context.agentId) turn \(context.turn): \(context.friendlyDivisions.count) friendly formations, \(context.enemyDivisions.count) known enemy formations, \(context.objectives.count) objectives visible."
    }
