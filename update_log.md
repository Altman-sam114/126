...
- 2026-07-07：新增战争支持影响 Cabinet 姿态和 AI 外交回应的最小反馈。`RulerAgent` 的 `RulerStrategicSnapshot` 读取 faction 平均 `CountryProfile.warSupport`，低支持度会提高 `attackThresholdAdjustment`、增加 `reserveBias`，并把激进攻势塑形成 defensive / stabilize posture；`diplomaticPlayResponses` 读取当前 ruler country warSupport，支持度临界时选择 `neutral` 并在 rationale 中说明避免升级，支持度较高时仍可按关系分数支持 issuer / target。该切片只影响 AI 姿态、阈值、预备队倾向和外交回应解释，不直接修改 `warSupport`、外交关系、经济账本、hex/region controller 或单位位置；完整条件交换、多方谈判、停战、新闻舆论和动态战争目标仍待后续切片。
