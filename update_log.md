...
- 2026-07-06：接入外交面板受限宣战 UI 入口。`DiplomacyPanelView` 新增 `Crisis Actions` 区，从当前外交 country profile 派生目标，只在非 observer、active faction 可由玩家命令、action phase 且 `canDeclareWar` 通过时启用按钮；点击后经 `AppContainer.executeDiplomacyCommand` 提交 `Command.diplomacy(.declareWar)`，仍由 `RuleEngine` 最终校验和执行。`BoardInteractionTests` 增加容器命令路由与 observer 拒绝语义断言。本切片仍不是完整 DiplomaticPlay、谈判 UI 或动态战争目标系统。
- 2026-07-07：新增最小 `DiplomaticPlay` 创建闭环。`DiplomacyState` 保存 active plays，记录 issuer / target / region / warGoal / escalation / backers / opposingBackers / deadline / outcome，并对旧存档 JSON 缺失 `diplomaticPlays` 做空数组兼容；`Command.diplomacy(.createDiplomaticPlay)` 经 `CommandValidator` 限制 action phase、非自身/neutral、双方有 country profile、尚未 atWar、无重复 active play 且 region 存在后，只写入 play 状态和 `.diplomacy` 日志，不改变外交关系、hex/region controller、单位、前线、部署或经济账本。`DiplomacyPanelView` 展示 active plays，并提供受限 Open diplomatic play 按钮。本切片仍不包含支持者选择、逐回合升级、让步/谈判、自动开战或动态胜利条件。
- 2026-07-07：新增最小外交危机推进与到期升级规则。`CommandExecutor.executeEndTurn` 只在完整 `turnOrder` cycle 结束并进入新 turn 时调用 `DiplomacyState.advanceDiplomaticPlays`，每个 active play 增加 escalation；deadline 到期时通过 `DiplomacyState.declareWar` 尝试将 issuer / target 关系置为 `atWar`，宣战成功或双方已交战后把 play outcome 标为 `escalatedToWar`，宣战失败且双方仍未交战时保持 active，写入 `.diplomacy` 日志，并通过既有 end-turn runtime refresh 重建前线/部署派生层。该切片不直接移动单位、不改 hex/region controller、不改经济账本；仍未实现支持者选择、让步/谈判、停战或动态胜利条件。
- 2026-07-07：新增最小外交危机让步收束命令。`DiplomacyCommand.offerConcession(playId:)` 经 `CommandValidator` 限制 action phase、play 必须 active、active faction 必须是 issuer 或 target 且双方尚未 atWar 后，由 `DiplomacyState.offerConcession` 将 play outcome 标为 `negotiatedSettlement`，写入 `.diplomacy` 日志；`DiplomacyPanelView` 在 active play 上提供受限 `Offer concession` 按钮。该切片不改变外交关系、不调整 `CountryProfile.warSupport`、不移动单位、不改 hex/region controller、不刷新前线/部署、不改经济账本；仍未实现多方支持者选择、条件交换、停战或动态战争目标谈判。
- 2026-07-07：新增最小外交危机支持/反对列表加入命令。`DiplomacyCommand.supportDiplomaticPlay(playId:side:)` 经 `CommandValidator` 限制 action phase、play 必须 active、active faction 必须有 country profile、不是 issuer/target、尚未加入任一侧，且 issuer/target 与所支持一侧均未处于不合法战争关系后，由 `DiplomacyState.supportDiplomaticPlay` 只追加到 `backers` 或 `opposingBackers`、排序去重并写 `.diplomacy` 日志；`DiplomacyPanelView` 在 active play 上提供受限 `Back issuer` / `Back target` 按钮。该切片不改变外交关系、不自动参战、不调整 `CountryProfile.warSupport`、不移动单位、不改 hex/region controller、不刷新前线/部署、不改经济账本；仍未实现条件交换、停战或动态战争目标谈判。
- 2026-07-06：补齐外交危机支持阵营与运行时派生层整合。`DiplomacyState.advanceDiplomaticPlays` 在 deadline 到期时不再只升级 issuer / target，而是按 `backers × opposingBackers` 逐对调用 `declareWar`，使支持方真正进入后续 `canAttack` / 前线 / 部署判断；直接宣战会立即关闭双方跨侧的 active play，避免 UI 留下已开战但仍 active 的危机。`AppContainer.submit` 在成功命令后统一调用 runtime refresh，再做玩家命令 bookkeeping 和将军分配刷新，避免移动、攻击或外交状态变化后前线/部署显示滞后。本切片不直接移动单位、不改 hex / region controller、不改经济账本；完整外交谈判仍待后续切片。
- 2026-07-07：新增最小 AI 外交危机回应与解释记录。`DiplomaticPlay` 增加 `aiStanceRecords` 且旧 JSON 缺字段时默认空数组；`DiplomacyCommand.respondToDiplomaticPlay(playId:stance:agentId:rationale:)` 经 `CommandValidator` 限制 active play、非 issuer/target、非 neutral、有 country profile、尚未加入任一侧且双方尚未 atWar 后执行。`supportIssuer` / `supportTarget` 复用 `DiplomacyState.supportDiplomaticPlay` 写入支持列表，`neutral` 只记录 `DiplomaticPlayStanceRecord` 和 `.diplomacy` 日志。`RulerAgent.diplomaticPlayResponses` 在默认 AI action phase 生成 deterministic stance/rationale，`TurnManager` 在 war directive 和 endTurn 前把它转成规则命令执行；`DiplomacyPanelView` 展示最近 AI stance。该切片不直接改变外交关系、不自动参战、不移动单位、不改 hex/region controller、不改经济账本；完整条件交换、多方谈判、停战、新闻舆论和动态战争目标仍待后续切片。功能 commit `7562dd8d460b7bccbd2807cf40cde1d225afbc4a` 已由 GitHub Actions run `28839829616` attempt `1` 的 artifact `WWIIHexV0-ci-v1-main-7562dd8-run28839829616-attempt1` 核对通过，manifest 显示 `branch=main`、`commitSha=7562dd8d460b7bccbd2807cf40cde1d225afbc4a`、`staticChecksOutcome=success`、`buildOutcome=success`、`testOutcome=skipped`；`junit.xml` 为 3 tests、0 failures、1 skipped；`xcodebuild.log` 结尾 `BUILD SUCCEEDED`。
- 2026-07-07：新增战争支持影响 Cabinet 姿态和 AI 外交回应的最小反馈。`RulerAgent` 的 `RulerStrategicSnapshot` 读取 faction 平均 `CountryProfile.warSupport`，低支持度会提高 `attackThresholdAdjustment`、增加 `reserveBias`，并把激进攻势塑形成 defensive / stabilize posture；`diplomaticPlayResponses` 读取当前 ruler country warSupport，支持度临界时选择 `neutral` 并在 rationale 中说明避免升级，支持度较高时仍可按关系分数支持 issuer / target。该切片只影响 AI 姿态、阈值、预备队倾向和外交回应解释，不直接修改 `warSupport`、外交关系、经济账本、hex/region controller 或单位位置；完整条件交换、多方谈判、停战、新闻舆论和动态战争目标仍待后续切片。功能 commit `f38218a264a603091c3a8214270828d836ab7d24` 已由 GitHub Actions run `28840881834` attempt `1` 的 artifact `WWIIHexV0-ci-v1-main-f38218a-run28840881834-attempt1` 核对通过，manifest 显示 `branch=main`、`commitSha=f38218a264a603091c3a8214270828d836ab7d24`、`staticChecksOutcome=success`、`buildOutcome=success`、`testOutcome=skipped`；`junit.xml` 为 3 tests、0 failures、1 skipped；`xcodebuild.log` 结尾 `BUILD SUCCEEDED`。
- 2026-07-07：新增外交危机让步结算后果。`DiplomaticPlay` 增加旧 JSON 兼容的 `settlementRecord`，`offerConcession` 收束 play 时记录让步方、受益方、warGoal、summary 和 warSupport delta；当前最小规则意图为让步方 `-4`、受益方 `+2`，实际记录按 `CountryProfile.warSupport` 的 0-100 钳制后变化写入，并阻止同一双方/region 在同回合立即重开已谈判收束的危机，下一回合可重新开启。`DiplomacyPanelView` 的 Open diplomatic play 按钮同步读取 turn-aware cooldown。该切片仍不改变外交关系、hex/region controller、单位、前线/部署或经济账本；完整条款列表、赔款/割地、停战系统和动态战争目标仍待后续切片。功能 commit `4588ae7f4845077dc890871e20e535db4b304d4e` 已由 GitHub Actions run `28842050330` attempt `1` 的 artifact `WWIIHexV0-ci-v1-main-4588ae7-run28842050330-attempt1` 核对通过，manifest 显示 `branch=main`、`commitSha=4588ae7f4845077dc890871e20e535db4b304d4e`、`staticChecksOutcome=success`、`buildOutcome=success`、`testOutcome=skipped`；`junit.xml` 为 3 tests、0 failures、1 skipped；`xcodebuild.log` 结尾 `BUILD SUCCEEDED`。
- 2026-07-07：新增外交危机战争目标到胜利规则的最小桥。`DiplomaticPlayWarGoal` 增加 objective-backed 映射，`VictoryRules` 在 scenario 静态胜利条件未立即判胜时，会检查 outcome 为 `escalatedToWar` 的 diplomatic play；若 issuer side 控制该 warGoal 对应的现有地图 objectives，则以 `.diplomaticWarGoalAchieved` 结算 issuer 胜利并记录 `resolvedConditionId=diplomatic_<playId>`。`CommandExecutor.executeEndTurn` 在整轮推进外交危机后补一次胜利评价，使 deadline 当回合升级为战争的目标可立即判定；直接 `declareWar` 关闭 active play 并刷新前线/部署后也补跑胜利评价，使已满足的 dynamic warGoal 可立即结算。`DiplomacyPanelView` 对 active play 和尚未解决的 escalated objective-backed play 显示已有地图 objective group 的 `Victory stake`，非 active play 不显示支持/让步按钮。该切片仍不新增动态 victoryCondition 存档 schema，不让外交目标直接改 hex/region controller，不处理 `weakenPrestige` 的威望胜利，也不是完整条件交换、停战或谈判系统。功能 commit `0ff3ffedcf3c6c24dd68c11782da9560ae479261` 已由 GitHub Actions run `28844323191` attempt `1` 的 artifact `WWIIHexV0-ci-v1-main-0ff3ffe-run28844323191-attempt1` 核对通过，manifest 显示 `branch=main`、`commitSha=0ff3ffedcf3c6c24dd68c11782da9560ae479261`、`staticChecksOutcome=success`、`buildOutcome=success`、`testOutcome=skipped`；`junit.xml` 为 3 tests、0 failures、1 skipped；`xcodebuild.log` 结尾 `BUILD SUCCEEDED`。
- 2026-07-07：新增 `weakenPrestige` 威望压力战争目标的最小 warSupport 胜利桥。`DiplomaticPlayWarGoal.weakenPrestige` 暴露 `warSupportVictoryThreshold=35`；`VictoryRules` 对 outcome 为 `escalatedToWar` 的 diplomatic play 先检查目标方主国家 `CountryProfile.warSupport`，低于或等于阈值时以 `.diplomaticWarGoalAchieved` 结算 issuer 胜利并记录 `resolvedConditionId=diplomatic_<playId>`。`DiplomacyPanelView` 对 active play 和尚未解决的 escalated play 显示 `Victory stake: <country> war support <= 35`，让玩家能看到该目标条件。该切片不新增 prestige / infamy 资源，不让 warSupport 自动结束所有战争，不处理目标方 backers 崩溃、停战、赔款、新闻舆论或完整谈判条款。功能 commit `f4e696262061d421bf1031f92e93fc5d9e1413b8` 已由 GitHub Actions run `28845455558` attempt `1` 的 artifact `WWIIHexV0-ci-v1-main-f4e6962-run28845455558-attempt1` 核对通过，manifest 显示 `branch=main`、`commitSha=f4e696262061d421bf1031f92e93fc5d9e1413b8`、`staticChecksOutcome=success`、`buildOutcome=success`、`testOutcome=skipped`；`junit.xml` 为 3 tests、0 failures、1 skipped；`xcodebuild.log` 结尾 `BUILD SUCCEEDED`。
- 2026-07-07：新增已升级外交危机战争的最小停战结算。`DiplomacyCommand.negotiateTruce(playId:)` 经 `CommandValidator` 限制 action phase、play 必须为 `escalatedToWar`、active faction 必须是 issuer 或 target 且双方阵营仍有 cross-side `atWar` 后，由 `DiplomacyState.negotiateTruce` 只把 issuer side / target side 中真实处于 `.atWar` 的 cross-side 国家关系写为 `.truce`，把 play outcome 标为 `truceSettlement`，并由 `CommandExecutor` 写外交日志、刷新 `FrontLineState` / `WarDeploymentState` 派生层。`canDeclareWar` 和 `canCreateDiplomaticPlay` 同步把 `truce` 作为阻断条件，避免停战后立即直接宣战或重开同一危机；`DiplomacyPanelView` 对已升级且未结算的 play 显示受限 `Negotiate truce` 按钮。该切片不移动单位、不改 hex / region controller、不改经济账本、不处理赔款、割地、战争目标交换、目标方 backers 崩溃、新闻舆论或完整和平会议。功能 commit `4aa8c9715ec8f5124e0924c835c073b140fedf66` 已由 GitHub Actions run `28846454883` attempt `1` 的 artifact `WWIIHexV0-ci-v1-main-4aa8c97-run28846454883-attempt1` 核对通过，manifest 显示 `branch=main`、`commitSha=4aa8c9715ec8f5124e0924c835c073b140fedf66`、`staticChecksOutcome=success`、`buildOutcome=success`、`testOutcome=skipped`；`junit.xml` 为 3 tests、0 failures、1 skipped；`xcodebuild.log` 结尾 `BUILD SUCCEEDED`。
- 2026-07-07：新增停战期限冷却。`DiplomacyState.defaultTruceDuration=2`，并通过现有 `DiplomaticRelation.sinceTurn` 派生 `truceExpiresTurn` / `truceIsActive`，使 `canDeclareWar` 与 `canCreateDiplomaticPlay` 只在当前 turn 仍未超过 `sinceTurn + 2` 时阻断直接宣战和重开外交危机；冷却期过后保留 `.truce` 记录但不继续锁死外交。`DiplomacyPanelView` 的 crisis target 摘要会显示 `Truce until turn N`。该切片不新增 treaty JSON schema、不改变停战关系的存档字段、不移动单位、不改 hex / region controller、不改经济账本；完整停战条款、期限谈判、赔款/割地和和平会议仍待后续切片。功能 commit `e968f994b1631263c7e710e3a348af4f17f40b5c` 已由 GitHub Actions run `28847523420` attempt `1` 的 artifact `WWIIHexV0-ci-v1-main-e968f99-run28847523420-attempt1` 核对通过，manifest 显示 `branch=main`、`commitSha=e968f994b1631263c7e710e3a348af4f17f40b5c`、`staticChecksOutcome=success`、`buildOutcome=success`、`testOutcome=skipped`；`junit.xml` 为 3 tests、0 failures、1 skipped；`xcodebuild.log` 结尾 `BUILD SUCCEEDED`。
- 2026-07-07：新增停战冷却对第三方外交支持站队的约束。`canSupportDiplomaticPlay` / `canRespondToDiplomaticPlay` 改为 turn-aware gate，若 play 主当事双方仍在有效停战冷却期，或 active faction 与该 play 任一主当事方仍在有效停战冷却期，则不能通过 `supportDiplomaticPlay` 或 AI support/oppose stance 加入 `backers` / `opposingBackers`；AI 中立解释仍可只写 stance record。该切片延续 `sinceTurn + 2` 的最小冷却语义，不新增 treaty / peace conference schema，不改变外交关系、不移动单位、不改 hex / region controller、不改经济账本；完整停战条款、多方和平谈判、赔款/割地和新闻舆论仍待后续切片。功能 commit `809913ba8af920db6070da8c9371536e398e35b7` 已由 GitHub Actions run `28850573403` attempt `1` 的 artifact `WWIIHexV0-ci-v1-main-809913b-run28850573403-attempt1` 核对通过，manifest 显示 `branch=main`、`commitSha=809913ba8af920db6070da8c9371536e398e35b7`、`staticChecksOutcome=success`、`buildOutcome=success`、`testOutcome=skipped`；`junit.xml` 为 3 tests、0 failures、1 skipped；`xcodebuild.log` 结尾 `BUILD SUCCEEDED`。
- 2026-07-07：新增远征 depot 工程和补给锚点切片。`ConstructionKind.expeditionaryDepotWorks` 通过现有 `Command.queueConstruction -> CommandValidator -> RuleEngine -> EconomyRules` 入队，站点要求目标 hex 为己控、可通行且已有 `.coast` 或 `.port`；完成时只给目标 hex 添加 `.expeditionaryDepot` 物流标签。`SupplyRules` 把本方、allied 或 coBelligerent 控制的 `.expeditionaryDepot` 与港口一起纳入补给和安全撤退锚点，单纯 `militaryAccess` 仍不算共同补给。该切片不新增海军战术、登陆状态、全球殖民系统或 off-map sea lane，不改变 hex / region controller、不改战区归属、不直接移动单位。功能 commit `721be1365cc9deebf84e3671c45db8a3bff57b9a` 已由 GitHub Actions run `28855175398` attempt `1` 的 artifact `WWIIHexV0-ci-v1-main-721be13-run28855175398-attempt1` 核对通过，manifest 显示 `branch=main`、`commitSha=721be1365cc9deebf84e3671c45db8a3bff57b9a`、`staticChecksOutcome=success`、`buildOutcome=success`、`testOutcome=skipped`；`junit.xml` 为 3 tests、0 failures、1 skipped；`xcodebuild.log` 结尾 `BUILD SUCCEEDED`。
- 2026-07-07：新增最小封锁外交命令切片。`DiplomacyCommand.imposeBlockade(targetFaction:)` 通过现有 `Command.diplomacy -> CommandValidator -> RuleEngine -> CommandExecutor -> DiplomacyState` 管线执行，校验 action phase、目标非自身/非 neutral、双方有 country profile、当前关系不是 allied / coBelligerent / militaryAccess / atWar / blockaded，且不处于有效 truce 冷却；成功后把合法目标关系写为 `.blockaded`、记录 `.diplomacy` 日志并刷新运行时派生层。`.blockaded` 不允许攻击或进境，并复用 `MapState.logisticsAnchorCoords` 的外交门控，让目标控制的港口和远征 depot 不再作为封锁方补给锚点。该切片不移动单位、不改变 hex / region controller、不改经济账本、不做海军战术、港口吞吐、市场价格、补给惩罚或 off-map sea lane；后续经济工业和科技变迁仍应优先围绕国库、工业产能、铁路运输力、船运量和科技/改革修正形成可玩取舍。本轮 GitHub Actions 结果包尚待 push 后由 Agent C 核对。
