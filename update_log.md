...
- 2026-07-07：新增已升级外交危机战争的最小停战结算。`DiplomacyCommand.negotiateTruce(playId:)` 经 `CommandValidator` 限制 action phase、play 必须为 `escalatedToWar`、active faction 必须是 issuer 或 target 且双方阵营仍有 cross-side `atWar` 后，由 `DiplomacyState.negotiateTruce` 把 issuer side / target side 的全部 cross-side 国家关系写为 `.truce`，把 play outcome 标为 `truceSettlement`，并由 `CommandExecutor` 写外交日志、刷新 `FrontLineState` / `WarDeploymentState` 派生层。`canDeclareWar` 和 `canCreateDiplomaticPlay` 同步把 `truce` 作为阻断条件，避免停战后立即直接宣战或重开同一危机；`DiplomacyPanelView` 对已升级且未结算的 play 显示受限 `Negotiate truce` 按钮。该切片不移动单位、不改 hex / region controller、不改经济账本、不处理赔款、割地、战争目标交换、目标方 backers 崩溃、新闻舆论或完整和平会议。
